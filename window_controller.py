...
class WindowController:
    def __init__(self, max_ips="auto"):
        ...
        try:
            ...
            # [#41] Rate-limite les tentatives de reconnexion automatique
            # declenchees par un flux fige (voir screenshot()) : on ne veut
            # pas spammer reconnect_scrcpy() a chaque frame tant que le flux
            # reste fige.
            self._last_auto_reconnect_attempt = 0.0
            self._auto_reconnect_cooldown = 20.0
    ...
    def screenshot(self):
        ...
        if frame_time > 0 and age > self.FRAME_STALE_TIMEOUT:
            ...
            # [#41 Recovery system] : un flux fige depuis trop longtemps ne
            # se repare jamais tout seul -- on tente une reconnexion
            # automatique, avec un cooldown pour ne pas boucler dessus.
            now = time.time()
            if now - self._last_auto_reconnect_attempt > self._auto_reconnect_cooldown:
                self._last_auto_reconnect_attempt = now
                print("Frame feed stale for too long, attempting automatic scrcpy reconnect...")
                try:
                    self.reconnect_scrcpy()
                except Exception as exc:
                    print(f"Automatic reconnect attempt failed: {exc}")
                frame, frame_time = self.get_latest_frame()
                if frame is None:
                    raise ConnectionError(
                        "Scrcpy feed was stale and automatic reconnect did not restore a frame."
                    )
    ...
    def press(self, key, delay=0.02, touch_up=True, touch_down=True):
        if key not in press_coords_dict:
            # [#48] Une touche manquante dans buttons_config.toml passait
            # inapercue avant (retour silencieux) : on log pour que ce soit
            # visible en debug au lieu de simplement "ne rien faire".
            print(f"WARNING: press('{key}') ignored -- '{key}' is not defined in buttons_config.toml.")
            return
