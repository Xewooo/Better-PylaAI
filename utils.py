...
def invalidate_toml_cache(file_path):
    full_path = PROJECT_ROOT / str(file_path).lstrip('/\\')
    # .pop(..., None) instead of `del` : appeler cette fonction pour un
    # fichier jamais mis en cache (ou deja invalide) ne doit pas planter le
    # bot avec un KeyError (#48 - gestion d'erreur des composants critiques).
...
SAFE_GLOBALS = {
    ...
    # --- Ajouts (playstyles .pyla plus avances) -------------------------
    # Ces ajouts sont des builtins de pure manipulation de donnees (aucun
    # I/O, aucune execution dynamique de code, aucun acces au systeme de
    # fichiers) : ils ne reduisent en rien la surface de securite deja
    # assuree par is_safe_ast (imports interdits, attributs '_'-prefixes
    # interdits, et exec/eval/compile/getattr/setattr/delattr/__import__
    # explicitement bloques a l'analyse du code, avant meme la compilation).
    #
    # Sans ces noms, un script .pyla qui fait `for i, x in enumerate(...)`
    # ou `except Exception:` plante avec un NameError des qu'il s'execute
    # reellement (le NameError n'apparait pas forcement tout de suite : il
    # ne se declenche que la frame ou cette ligne de code est effectivement
    # atteinte, ce qui le rend facile a manquer en test rapide).
    'bool': bool,
    'list': list,
    'dict': dict,
    'tuple': tuple,
    ...
    'divmod': divmod,
    'pow': pow,
    # Types d'exception standards, pour permettre un `except <Type>:` cible
    # plutot qu'un `except:` nu (qui capture aussi de vraies erreurs de
    # logique dans le script et les rend silencieuses).
    'Exception': Exception,
